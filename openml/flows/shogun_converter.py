...
class ShogunConverter(AbstractConverter):
    def __init__(self, model: sg.machine):
    ...
    def _flow_to_shogun(o, components=None, initialize_with_defaults=False):
        """Initializes a shogun model based on a flow.
        ...
        if isinstance(o, dict):
            ...
            if 'oml-python:serialized_object' in o:
                ...
                if serialized_type == 'type':
                    rval = ShogunConverter._deserialize_type(value)
                elif serialized_type == 'rv_frozen':
                    # rval = ShogunConverter._deserialize_rv_frozen(value)
                    raise NotImplementedError("Cannot handle rv frozen")
                elif serialized_type == 'function':
                    rval = ShogunConverter._deserialize_function(value)
                elif serialized_type == 'component_reference':
                    value = ShogunConverter._flow_to_shogun(value)
                    step_name = value['step_name']
                    key = value['key']
                    component = ShogunConverter._flow_to_shogun(components[key],
                    ...
                    rval = ShogunConverter._deserialize_cross_validator(value)
                else:
                    raise ValueError('Cannot flow_to_shogun %s' % serialized_type)

            else:
                rval = OrderedDict((ShogunConverter._flow_to_shogun(key, components, initialize_with_defaults),
                                    ShogunConverter._flow_to_shogun(value, components, initialize_with_defaults))
                                   for key, value in sorted(o.items()))
        elif isinstance(o, (list, tuple)):
            rval = [ShogunConverter._flow_to_shogun(element, components, initialize_with_defaults) for element in o]
    ...
    def _shogun_to_flow(o, parent_model=None):
        ...
        if ShogunConverter._is_estimator(o):
            ...
            # if isinstance(o, tuple):
            #     rval = tuple(rval)
            raise NotImplementedError("_shogun_to_flow: isinstance(o, (list, tuple) not implemented")
        elif isinstance(o, (bool, int, float, six.string_types)) or o is None:
            ...
            #     rval[key] = value
            # rval = rval
            raise NotImplementedError("_shogun_to_flow: isinstance(o, dict) not implemented")
        elif isinstance(o, type):
            # TODO: explain what type of parameter is here
            # rval = SKLearnConverter._serialize_type(o)
            raise NotImplementedError("_shogun_to_flow: isinstance(o, type) not implemented")
        ...
        # built-in or functool.partials in a pipeline
            ...
            # TODO: explain what type of parameter is here
            rval = ShogunConverter._serialize_function(o)
        elif ShogunConverter._is_cross_validator(o):
            # TODO: explain what type of parameter is here
            rval = ShogunConverter._serialize_cross_validator(o)
        else:
    ...
    def to_flow(self):
        ...
        # in shogun all classes are in the top module
        ...
        dependencies = [self.format_external_version('shogun', ""),
                        'numpy>=1.7.0']
        dependencies = '\n'.join(dependencies)
    ...
    def extract_information_from_model(self):
        ...
        # TODO add support for pipelines and parameter optimisation

        if self._model == "Pipeline":
            raise NotImplementedError("Need to implement pipelines")
        else:
            self._parameters = _get_shogun_param_values(self._model)
            for k, v in self._parameters.items():
                description = self._model.parameter_description(k)
                value_type = self._model.parameter_type(k)
                self._parameters_meta_info[k] = OrderedDict((('description', description),
    ...
    def _extract_shogun_model_information(self, rval, parameter_name):
    ...
    #     raise NotImplementedError("check_multiple_occurence_of_component_in_flow not implemented")
    ...
    def _extract_openml_flow_information(self, rval, parameter_name):
        ...
        component_reference['value'] = cr_value
        component_reference = self._shogun_to_flow(component_reference, self._model)
    ...
    def external_version(self):
        ...
        model_package_name = self._model.__class__.__module__.split('.')[0]
        module = importlib.import_module(model_package_name)
        model_package_version_number = module.get_global_version().get_version_release()
    ...
    def _deserialize_cross_validator(value):
        ...
        for parameter in parameters:
            parameters[parameter] = ShogunConverter._flow_to_shogun(parameters[parameter])
        return model_class(**parameters)


#    def run_on_task(self, task):

def _get_shogun_param_values(obj):
    result = OrderedDict()
    for name in obj.parameter_names():
        try:
